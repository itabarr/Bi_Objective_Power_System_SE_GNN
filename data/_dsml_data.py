...
def get_bus_param(net):
    ...
    ----------
    net : object
        Network object containing bus and load information.

    Returns
    -------
    df_bus_param : pandas.DataFrame
        DataFrame with columns:
        - 'vn_kv': nominal voltage of each bus,
        - 'bool_slack': 1 if bus is slack, 0 otherwise,
        - 'bool_zero_inj': 1 if bus is zero injection, 0 otherwise.
    """
...
def get_edge_param(net):
    """
    Extracts edge (line and transformer) parameters from a given network object.

    Parameters
    ----------
    net : object
        Network object containing line and transformer information.

    Returns
    -------
    edge_param : pandas.DataFrame
        DataFrame with edge parameters including admittance, phase shift, and limits.
    """
...
def data_from_pickles(folder, num_nfeat, num_efeat, num_nmeas, num_emeas, meas_v, meas_pflow):
    """
    Loads node, edge, label, and noise data from pickle files and constructs
    PyTorch Geometric Data objects with normalized features.

    Parameters
    ----------
    folder : str
        Path to the folder containing the pickled data files.
    num_nfeat : int
    ...
    num_emeas : int
        Number of edge measurements.
    meas_v : list or array-like
        Indices of buses with voltage measurements.
    meas_pflow : list or array-like
        Indices of edges with power flow measurements.

    Returns
    -------
    data_list : list of torch_geometric.data.Data
        List of Data objects for each sample.
    x_set_mean : torch.Tensor
        Mean of node features (for normalization).
    x_set_std : torch.Tensor
        Standard deviation of node features.
    edge_attr_set_mean : torch.Tensor
        Mean of edge features.
    edge_attr_set_std : torch.Tensor
...
def get_pflow(y, edge_index, node_param, edge_param, phase_shift=True):
    """
    Computes power flow quantities (P, Q, I, loading) for each edge based on node outputs.

    Parameters
    ----------
    y : torch.Tensor
        Node outputs (voltage magnitude and angle).
    edge_index : torch.Tensor
        Edge indices (source and target nodes).
    node_param : torch.Tensor
        Node parameters (e.g., voltage levels).
    edge_param : torch.Tensor
        Edge parameters (admittance, phase shift, etc.).
    phase_shift : bool, optional
        Whether to include phase shift in calculation (default: True).

    Returns
    -------
    loading_lines : torch.Tensor
        Loading of lines.
    loading_trafo : torch.Tensor
        Loading of transformers.
    P_ij_from : torch.Tensor
        Active power flow from bus i to j.
    Q_ij_from : torch.Tensor
        Reactive power flow from bus i to j.
    P_ij_to : torch.Tensor
        Active power flow from bus j to i.
    Q_ij_to : torch.Tensor
        Reactive power flow from bus j to i.
    I_ij_from : torch.Tensor
        Current magnitude from bus i to j.
    I_ij_to : torch.Tensor
        Current magnitude from bus j to i.
    """
    V_n = node_param[:,0]
    V_hv = V_n.max()
...
def gsp_wls_edge(input, edge_input, output, x_mean, x_std, edge_mean, edge_std, edge_index, reg_coefs, num_samples, node_param, edge_param):
    ...
    including regularization terms for voltage, angle, and loading.

    Parameters
    ----------
    input : torch.Tensor
        Node input features (measurements and covariances).
    edge_input : torch.Tensor
        Edge input features (measurements and covariances).
    output : torch.Tensor
        Model output for nodes.
    x_mean : torch.Tensor
...
def gsp_wls(input, output, x_mean, x_std, edge_index, reg_coefs, grid, num_samples):
    ...
    reg_coefs : dict
        Regularization coefficients.
    grid : str
        Grid type identifier (e.g., 'cigre14', 'ober', 'lv').
    num_samples : int
        Number of samples in the batch.
