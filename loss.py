...
class WLSLoss(nn.Module):
    ...
    def __init__(self, lambda_voltage: float = 1e-4,
        ...
        self.node_weights = torch.tensor([lambda_voltage, lambda_voltage, lambda_phase, lambda_phase])
    ...
    def forward(self, input_data: torch.Tensor, edge_input: torch.Tensor,
        ...
        theta_angles = theta_angles*(1.0 - node_param[:, 1:2])  # Enforce slack bus angle = 0
...
#     def forward(self, input, edge_input, output, x_mean, x_std, edge_mean, edge_std,
#                 edge_index, node_param, edge_param):
#         """
#         Computes only the weighted least squares (WLS) loss for node and edge measurements.
#         """
#         total_nodes = input.shape[0]

#         # Extract measurements
#         z = input[:, ::2]          # V, theta, P, Q (nodes)
#         edge_z = edge_input[:, :4:2]  # Pflow, Qflow (edges)
...
#         # Model outputs (denormalized)
#         v_i = output[:, 0:1] * x_std[:1] + x_mean[:1]
#         theta_i = output[:, 1:]
#         theta_i = theta_i*(1. - node_param[:, 1:2])  # Enforce slack = 0

#         # Compute power flow
#         _, _, p_from, q_from, p_to, q_to, _, _ = get_pflow(
#             torch.cat([v_i, theta_i], dim=1), edge_index, node_param, edge_param
#         )
...
#         # Construct predicted measurements
#         h = torch.cat([v_i, theta_i, p_i.unsqueeze(1), q_i.unsqueeze(1)], dim=1)
#         h_edge = torch.cat([p_from.unsqueeze(1), q_from.unsqueeze(1)], dim=1)

#         # Residuals
#         delta = Z - h
#         delta[:, 1:2] = angular_distance(Z[:, 1:2], h[:, 1:2])  # wrap angles
#         delta_edge = edge_Z - h_edge

#         # Weighted least squares terms
#         meas_node_weights = torch.tensor(
#             [self.reg_coefs['lam_voltage'], self.reg_coefs['lam_voltage'], self.reg_coefs['lam_phase'], self.reg_coefs['lam_phase']],
#             device=output.device
#         )
#         meas_edge_weights = torch.tensor(
#             [self.reg_coefs['lam_power_flow'], self.reg_coefs['lam_power_flow']],
#             device=output.device
#         )

#         J_sample = torch.sum(delta**2 * R_inv * meas_node_weights, dim=1)
#         J_sample_edge = torch.sum(delta_edge**2 * R_edge_inv * meas_edge_weights, dim=1)

#         J = torch.mean(J_sample) + torch.mean(J_sample_edge)
#         return J

# class PhysicalLoss(nn.Module):
#     def __init__(self, reg_weight=1e2):
#         super(PhysicalLoss, self).__init__()
#         self.reg_weight = reg_weight
    
#     def forward(self, output, edge_index, node_param, edge_param, x_mean, x_std, reg_coefs=1e2):
#         """
#         Computes only the physical regularization terms:
#         - Voltage violation (J_v)
#         - Angle difference violation (J_theta)
#         - Loading violation (J_loading)
#         """
#         # Denormalize outputs
#         v_i = output[:, 0:1] * x_std[:1] + x_mean[:1]
#         theta_i = output[:, 1:]
...
#         indices_to = edge_index[1]
#         theta_ij = torch.abs(
#             torch.gather(theta_i[:, 0], 0, indices_from) -
#             torch.gather(theta_i[:, 0], 0, indices_to)
#         )

#         # Regularization terms
#         J_v = reg_coefs * torch.mean((torch.relu(v_i - 1.1) + torch.relu(0.9 - v_i))**2)
#         J_theta = reg_coefs * torch.mean((torch.relu(theta_ij - 0.5))**2)
#         J_loading = reg_coefs * torch.mean((torch.relu(loading - 1.5))**2)

#         return {
#             'voltage': J_v,
#             'angle': J_theta,
#             'loading': J_loading,
#             'total': J_loading + J_theta + J_v
#         }
...
class CombinedWLSPhysicalLoss(nn.Module):
    ...
    def forward(self, input_data: torch.Tensor, edge_input: torch.Tensor,
        ...
        # return J_v, J_theta, J_loading

# def gsp_regularization_loss(output, edge_index, node_param, edge_param, x_mean, x_std, reg_coefs):
#     """
#     Computes only the physical regularization terms:
#     - Voltage violation (J_v)
#     - Angle difference violation (J_theta)
...
#     v_i = output[:, 0:1] * x_std[:1] + x_mean[:1]
#     theta_i = output[:, 1:]
#     theta_i *= (1. - node_param[:, 1:2])  # Enforce slack = 0
